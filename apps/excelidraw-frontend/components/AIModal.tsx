...
const AIModal: React.FC<AIModalProps> = ({ open, onClose, game, roomId, userId }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div ref={modalRef} className={`bg-neutral-900 rounded-2xl p-8 shadow-2xl w-full text-center border border-emerald-700 relative ${response ? "max-w-4xl" : "max-w-md"}`} style={response ? { minHeight: 500 } : {}}>
                <button className="absolute top-4 left-4 text-emerald-400 hover:text-white transition" onClick={() => { onClose(); resetModal(); }} aria-label="Close">
                    <X size={22} />
                </button>
                ...
                {!activeSection && (
                    <div className="flex flex-row gap-6 justify-center mb-4">
                        <button className="flex-1 bg-emerald-800/70 hover:bg-emerald-600 text-white rounded-xl p-6 font-semibold text-lg shadow transition-all duration-200" onClick={() => setActiveSection("object")}>
                            Draw an Object
                        </button>
                ...
                {activeSection === "object" && (
                    <form onSubmit={e => { e.preventDefault(); if (!loading && objectPrompt.trim()) handleSubmit("OBJECT", objectPrompt); }}>
                        <div className="text-left">
                            <button className="mb-4 text-emerald-400 hover:text-white transition font-semibold" type="button" onClick={() => { setActiveSection(null); setResponse(null); setError(null); }}>
                                ← Back
                            </button>
                        </div>
                        <div className="mb-6 text-left">
                            <label className="block text-emerald-300 font-semibold mb-2">What object should I draw?</label>
                        ...
                        <button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg font-semibold transition disabled:opacity-60" disabled={loading || !objectPrompt.trim()}>
                            {loading ? <span className="flex items-center gap-2"><Loader2 className="animate-spin" /> Generating...</span> : "Submit"}
                        </button>
                        {response && (
                            <div className="mt-4 flex flex-col items-center w-full">
                                <div className="text-emerald-300 font-semibold mb-2">Generated object for: <span className="text-white">{lastPrompt}</span></div>
                                <canvas ref={previewCanvasRef} width={480} height={320} style={{ background: "#222", borderRadius: 8, border: "1px solid #444" }} />
                                <button className="mt-4 px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold transition" onClick={insertIntoCanvas}>
                                    Insert into Canvas
                                </button>
                            </div>
                        )}
