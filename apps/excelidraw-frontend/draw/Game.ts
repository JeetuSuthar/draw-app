...
private convertPoints(apiShape: any): { x: number, y: number }[] | undefined {
        ...
        if (this.type === 'text') {
            ...
            if (input) {
                ...
                if (text.length > 0) {
                    ...
                    this.send(shape);
                    this.clicked = false;
                    // Optional: Add this line if text doesn't appear when switching tools
                    this.clearCanvas();
                }
    ...
    mouseUpHandler = (e: MouseEvent) => {
        ...
        // Add this line to redraw the canvas after adding a shape
        this.clearCanvas();
    }
        ...
        if (e.key == 'Enter') {
            ...
            if (input) {
                ...
                if (text.length > 0) {
                    ...
                    this.send(shape);
                    this.clicked = false;
                    // Add this line to redraw the canvas after adding the text shape
                    this.clearCanvas();
                }
                document.body.removeChild(input);
            }
